...
def setup():
    ...
    frameRate(120000000)
def draw():
    global x, g, h, u, i , l, c, c1, c2, c3, w, Sp, DB, SB, cB1, cB2, cB3, CH, CT, inf, pu
    background(255)
    i = loadImage("lox.jpg")
    w = loadImage("ntl.jpg")
    pu = loadImage("pult.jpg")
    fill(0)
    ...
    if key == 'l':
        inf=0
    if key == 'p':
        image(pu,0,0,2000,1000)
        text(u"ПУЛЬТ ОТ ЯДЕРКИ",500,500)
        inf=3
    if l == 1:
    ...
    if inf == 0:
        text(u"открыть обучение O",50,900)       
    if inf == 1:
       text(u"Жёлтый-моеты их нужно собрать 3 ,Чёрный - обрыв,Белый - земля,ходьба на стрелки",50,935)
       text(u"Зелёный-Баф скорости,сверху количество монет,закрыть обучение на клавмщу L",50,900)
       text(u"серый - туман в нем можно чтото найти например записки",50,865)
